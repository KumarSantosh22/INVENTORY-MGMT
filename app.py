...
def notify_stock():
    ...
    if 'username' in session:
        ...
        cursor.execute('SELECT * FROM shop where user_name=%s;', (user_name,))
...
def add():
    ...
    if request.method == 'POST':
        ...
        user_name = session.get('username')
        cursor = mysql.connection.cursor()
        cursor.execute('INSERT INTO shop(name, description, price, stock, total, reorder_level, user_name) VALUES(%s, %s, %s, %s, %s, %s, %s )',
                       (name, description, price, stock, total, reorder_level,user_name))
    ...
    if 'username' in session:
        if request.method == 'POST':
            ...
            if iid:
                ...
                cursor.close()
                notify_stock()
                return render_template('billing.html', item=item)
            else:
